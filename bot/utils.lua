...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['banned']) == nil then --fix way
     if not lang then
					...
					return "*لا يوجد محظورين في هاذا الكروب*"
              end
				end
       if not lang then
   message = '*List of banned users :*\n'
         else
   message = '_قائمة المحضورين  :_\n'
     end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['is_silent_users']) == nil then --fix way
        if not lang then
					...
					return "*لا يوجد مكتومين في هاذا الكروب*"
             end
				end
      if not lang then
...
local lang = redis:get(hash)
  ...
  if next(data['gban_users']) == nil then --fix way
    if not lang then
					...
					return "* لا يوجد محضورين عام*"
             end
				end
        if not lang then
   message = '*List of globally banned users :*\n'
   else
   message = '_قائمة المحضورين عام :_\n'
   end
...
local lang = redis:get(hash)
  ...
  if not lang then
    ...
    return 'لم يتم تفعيل البوت في هاذا الكروب'
   end
  end
  -- determine if table is empty
  ...
  if not data[tostring(msg.chat_id_)]['filterlist'] then
    ...
    save_data(_config.moderation.data, data)
      ...
      if not lang then
       ...
       filterlist = '_قائمة الكلمات المحضورة :_\n'
    end
 local i = 1
