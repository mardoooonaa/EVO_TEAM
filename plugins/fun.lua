...
local function get_weather(location)
 ...
 local temp = 'درجة حرارة🌟 '..city..' الان🌀 '..weather.main.temp..' درجة cْ \n➖➖➖➖➖➖\n @dev_evo :)'
 ...
 elseif weather.weather[1].main == 'Thunderstorm' then
  conditions = conditions .. 'شديد المطر ☔️☔️☔️☔️'
 elseif weather.weather[1].main == 'Mist' then
  conditions = conditions .. 'مايو 💨'
 end
 return temp .. '\n' .. conditions
end
--------------------------------
   ...
   function tophoto_cb(arg,data)
    if data.content_.sticker_ then
     ...
     if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
      ...
      tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, "@dev_evo", dl_cb, nil)
     else
   ...
   function tosticker_cb(arg,data)
    if data.content_.ID == 'MessagePhoto' then
     ...
     if file_exi(file..'_(1).jpg', tcpath..'/data/photo', 'jpg') then
      ...
      tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, pfile, '@dev_evo', dl_cb, nil)
     else
...
if matches[1] == 'voice' then
 local text = matches[2]
  ...
  local file = download_to_file(url,'EvO_DeV.mp3')
     tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, '@dev_evo', dl_cb, nil)
   end
end
...
--------------------------------
 if matches[1]:lower() == "photo" then 
  ...
  tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "BD-Reborn.mp3", dl_cb, nil)
 end


--------------------------------
if matches[1] == "الاوامر الملحقة" then
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
if not lang then
helpfun = [[
_Evo Reborn Fun Help Commands:_
...
helpfun = [[
_اوامر الملحقة بالبوت :_

*!time*
_ارسال الوقت على نمط الملسق_

*!short* `[link]`
_لـاختصار الرابط ب6 روابط_

*!voice* `[text]`
_لتبديل الكلام وتحويله الى صوت_

*!tr* `[lang]` `[word]`
_ترجمه من عربي الى انكليزي_
_مثال:_
_!tr en سلام_

*!sticker* `[word]`
_لتبديل الكلام الى ملسق_

*!photo* `[word]`
_تبديل الملسق الى صورة_

*!azan* `[city]`
_لمعرفة اوقات الاذان_

*!calc* `[number]`
_لاستخدام عمليات الرياضية_

*!praytime* `[city]`
_لمعرفة وقت مدينتك_

*!tosticker* `[reply]`
_لتبديل الصورة الى ملسق_

*!tophoto* `[reply]`
_لتبديل الملسق وبالعكس_

*!weather* `[city]`
_لمعرفة الطقص الخاص بمدينتك_

*يمكنك استخدام الاوامر الاتية حسب رتبتك عند البوت👨🏻‍💻✔️ *
