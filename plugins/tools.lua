...
local function warning(msg)
	...
	local d = math.floor(expiretime / 86400) + 1
        if tonumber(d) == 1 and not is_sudo(msg) and is_mod(msg) then
			if lang then
				tdcli.sendMessage(msg.to.id, 0, 1, 'من دعم المجموعه بقی یوم واحد ل دعم المجموعه من جدید تواصل معی مطور البوت واذا لم تدفع المجموعه سیتم حذفها من قائمه البوت وسیخرج البوت من المجموعه', 1, 'md')
			else
...
if not is_admin1(data.id_) then
   if not lang then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "🔬┋المعرف :"..user_name.."\n🔬┋الايدي :*"..data.id_.."*\n تم حذفه من قائمة المسؤولين", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
if already_sudo(tonumber(data.id_)) then
  ...
  if not lang then
   ...
   if not lang then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "🔬┋المعرف :"..user_name.."\n🔬┋الايدي :*"..data.id_.."*\n🚸┋ هو ليس ضمن قائمة المسؤولين", 0, "md")
      end
   end
          table.remove(_config.sudo_users, getindex( _config.sudo_users, tonumber(data.id_)))
		save_config()
...
local function pre_process(msg)
	if msg.to.type ~= 'pv' then
		...
		if gpst and not chex and msg.from.id ~= SUDO and not is_sudo(msg) then
			...
			if lang then
				tdcli.sendMessage(msg.to.id, msg.id_, 1, '_المجموعه تم دعمها لمدت یوم واحدرجان تواصل معی مطور البوت لزیادت الدعم غیر هذا سیتم حذف المجموعه من قائمه البوت وسیخرج البوت من المجموعه_', 1, 'md')
			else
...
text = [[
_دليل 📜 الادمنية والمطورين 📬 في بوت EVO والمزيد📮:_

			
*رفع مطور* `[username|id|reply]`
_اضافه مطور في البوت_

*تنزيل مطور* `[username|id|reply]`
_حذف مطور من البوت_

*المطورين* 
_لعرض قائمة المطورين_

*رفع ادمن* `[username|id|reply]`
_اضافة مطور في المجموعات_

*تنزيل مطور* `[username|id|reply]`
_لتنزيل مطور في المجموعات_

*الادمنيه* 
_لعرض قائمة الادمنيه_

*غادر* 
_لجعل البوت يغادر المجموعة_

*الخروج التلقائي* `[تعطيل/تفعيل]`
...
*ترقية المجموعة* `[text]`
_لتحويل الكروب سوبر وترقيته_

*سوبر كروب*
_ترقية المجموعة_

*المسؤولين*
_لعرض قائمة المسؤولين في البوت_

*دعوة* `[id]`
_لدخول البوت عن طريق ايدي الكروب_

*مسح* `[id]`
_حذف المجموعة عن طريق الايدي_

*دخول* `[link]`
_لدخول البوت عن طريق الرابط_
...
*علامة القراءة* `[تعطيل/تشغيل]`
_لتشغيل وتعطيل علامة قراءة البوت_

*اذاعه* `[text]`
_لارسال رساله لجميع كروبات البوت_

*نشر* `[text]` `[ايدي الكروب]`
...
*حفض* `[plugin name] [reply]`
_لحفض ملف معين_

*حفض ملف* `[address/filename] [reply]`
_حفظ ملف معين_

*المدة*
_لمعرفة صلاحية البوت_

*المدة* `[GroupID]`
_معرفة صلاحية المجموعة عن طريق ايدي الكروب_


*دعوة* `[GroupID]`
_دعوة لمجموعة معينة عن طريق البوت_
